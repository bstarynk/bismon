...
\index{AST} and their symbolic annotations), it should be re-usable
with additional efforts as a foundation for many other
aspects~\footnote{For an example, look into the ongoing \index{h2020@\emph{\textsc{h2020}}}
  \href{https://www.decoder-project.eu/}{\textsc{Decoder}}
  \index{decoder@\textsc{Decoder}} H2020
...
The \href{http://www.barrelfish.org/}{\textsc{Barrelfish} operating
  system} \index{Barrelfish OS@\textsc{Barelfish} operating system} (see \cite{gerber:2018:authorization,
...
\item UTF-8 encoded \textbf{string} (the bytes inside such
  ...
  \item boxed constant \textbf{double} \index{double} \index{floating point} (IEEE 754) floating
  ...
  \item \textbf{node}. A \index{node}{node} has an object connective,
    ...
    \index{transient connective
      object@\texttt{transient\_connective\_object}}
    \texttt{transient\_connective\_object} are not dumped so are
    transient. \index{transient!node} \index{node!transient}
  ...
  \item \textbf{closure}. A \index{closure}{closure} is like a node
    ...
    whose connective is an instance of \index{transient connective
      object@\texttt{transient\_connective\_object}}
    \texttt{transient\_connective\_object} are not dumped so are
    transient.
...
\begin{itemize}
  \item a constant \emph{unique} serial id (of about 128 bits), called
    ...
    \texttt{\_4ggW2XwfXdp\_1XRSvOvZqTC}~\footnote{That objid
      ...
      numbers 3577488711679049683 {\relsize{-1}{(encoded in base  $62 = 10 + 2 \times 26$ as
      \texttt{4ggW2XwfXdp})}}, 1649751471969277032 i.e. to 128 bits
  ...
  \item a \index{space}{\textbf{space}} number fitting in a single
    ...
    \item the \index{component}{\emph{components}} of an object are
      ...
      \item objects may also have some (nearly arbitrary)
        ...
        \item mutable JSON \index{JSON} data~\footnote{That is, \texttt{json\_t*} from the \emph{Jansson} library. It is practically useful for WebSocket messages.} 
        \item etc...
        \end{enumerate}
...
The ability to have \emph{arbitrary} attributes and components in
...
\index{RefPerSys project@\textsc{RefPerSys} project}
\href{http://refpersys.org/}{\textsc{RefPerSys}}.  \index{load} It
always starts by loading some previous persisted \index{state}{state},
and usually dumps \index{dump} its current state before
...
depth-first \index{depth-first} \index{exploration!depth-first}
\index{search!depth-first} exploration \index{algorithm} algorithm
\index{decision!algorithm} (see also \cite{Cormen:2009:Intro-algo,
  Christian:2017:algorithms}) used to dump \index{dump} a persistent
\index{persistent state} state, and classical copying or
\href{https://en.wikipedia.org/wiki/Tracing\_garbage\_collection}{tracing
  garbage collection} \index{garbage collection}
\index{tracing!garbage collection} \index{copying!garbage collection}
...
\index{bootstrap} systems~\footnote{Observe that an entire Linux
  ...
  for guidance.} such as \textsc{Caia} \index{CAIA@\textsc{Caia}}
      {\small (see \cite{Pitrat:2000:machine-intelligence, Pitrat:2009:AST, Pitrat:2009:ArtifBeings},  \cite{Pitrat:blog}\footnote{That \href{http://bootstrappingartificialintelligence.fr/WordPress3/}{blog} explains that all the
      ...
      \index{Ocaml@\textsc{Ocaml}} {\small (cf \cite{Ocaml,
          Leroy-modular-modules} and many other papers by Xavier Leroy
        and the \href{http://gallium.inria.fr/}{Gallium} team at
        INRIA)}, \index{chicken scheme@\textsc{Chicken Scheme}} \index{Scheme@\emph{Scheme}} \textsc{Chicken/Scheme}\footnote{Notice that
        \emph{semantically} \index{semantics} Bismon is quite close to \emph{Scheme} and
        shares many features with that language; see
        \bmurl{http://starynkevitch.net/Basile/guile-tutorial-1.html}
        for more. The \index{bootstrap} bootstrapped \textsc{Chicken}\index{Chicken@\textsc{Chicken}}  system is on
        \bmurl{https://www.call-cc.org/} and is, like Bismon,
        translated to C.}, \textsc{Self} \index{Self@\textsc{Self}}
            {\small (cf \cite{Ungar:1987:Self})},
            \textsc{Sbcl}\footnote{See \bmurl{http://sbcl.org/} for
              more} \index{Sbcl@\textsc{Sbcl}} and \textsc{Clasp}
            \index{Clasp@\textsc{Clasp}} {\small (cf
            ...
            bootstrapped \index{bootstrap} system can be improved in
            ...
            improved (cf. \index{partial evaluation}{partial
              evaluation} and Futamura \index{projection} projections,
...
\begin{quote}
\begin{small}
%see https://tex.stackexchange.com/a/498475/42406
The lines starting with \index{«@\texttt{«}}\texttt{«} or
\index{"!(@\texttt{"!(}}\texttt{!(} and with
\index{»@\texttt{»}}\texttt{»} or \index{"!)@\texttt{"!)}}\texttt{!)}
...
comments in data files.} can start with a bar \texttt{|} till the
      ...
      \index{name}{\texttt{name}} of that object to
      \texttt{first\_test\_module}). Object payloads are also dumped
      in such ``modification'' form. \verb+!@+ puts the
      \index{mtime}{\emph{mtime}}. \verb+!$+ \textit{classobjid} sets
      the \index{class}{class} to the object of objid
      ...
      \index{"Σ@$\mathtt{\Sigma}$}
      $\mathtt{\Sigma}$ or
      %\index{"!"|@\texttt{!\textbar}}
      \verb+!|+
      is for \index{signature!function} function signatures.

         Within every dumped object, attributes whose class is
         \texttt{temporary\_attribute\_object} are \emph{not}
         \index{temporary attribute
           object@\texttt{temporary\_attribute\_object}} dumped. This
...
%% see also: https://tex.stackexchange.com/a/58811/42406
%%- \begin{quote}
%%-   \begin{relsize}{-1}
%%-   In July 2019, we have changed some lexical tokens
%%-   in data files.  Notice that, inside thes dumped files, \texttt{«} and
%%-   \verb'!(' for object opening, \texttt{»} and \verb'!)' for object
%%-   closing;
%%-   \texttt{µ} and \verb'!^' for modules,
%%-   $\mathtt{\Sigma}$ and \verb'!|'
%%-   for function signatures, \texttt{∈} and \verb'!$' for classes,
%%-   \texttt{±} and \verb'!@' for modtime, \texttt{↦} and \verb'!:' for
%%-   attributes, \texttt{↳} and \verb'!&' for components, are now strictly
%%-   equivalent. The motivation for doing that change is related to
%%-   \bmurl{https://stackoverflow.com/q/46113583/841108} \ldots
%%-   \end{relsize}
%%- \end{quote}
...
effort. At dump time, a \index{transient}{transient} object is
...
them (one data file per object space). A \emph{dumper} \index{dumper}
transient object -of class \texttt{dumper\_object} \index{dumper
  object@\texttt{dumper\_object}} is made to reify the ``global'' dump
status (notably the queue of objects to scan, and the hash-set of
dumped objects). So dumping happens by sending messages with selectors
-used by the dumping routine sending messages- like
\texttt{dump\_scan}, \texttt{dump\_value}, \texttt{dump\_data}.  The
\texttt{dump\_scan} \index{dump scan@\texttt{dump\_scan}} message is
sent\footnote{The \emph{dumper} object is the additional argument to
\texttt{dump\_scan}.} to a scanned object having some payload during
the scanning phase. The \texttt{dump\_value} \index{dump
  value@\texttt{dump\_value}} message is sent\footnote{Additional
arguments to to \texttt{dump\_value} are a string buffer object, the
\emph{dumper} object, and the depth as a tagged integer. Notice that
dumping of integers is implemented with the \texttt{dump\_value}
method of class \texttt{int}, dumping of tuples is implemented with
the \texttt{dump\_value} method of class \texttt{tuple}, dumping of
object references as objids \index{objid} is implemented with the
\texttt{dump\_value} method of class \texttt{object}, etc....} is to
dumped values during the emission phase.  The \texttt{dump\_data}
\index{dump data@\texttt{dump\_data}} message is sent\footnote{Extra
arguments to \texttt{dump\_value} are: the \emph{dumper} object and
the string buffer object.} when emitting an object content -notably
its payload - \index{payload} if relevant-, after having dumped its
class, attributes and components. But attributes \index{attribute}
which are direct or indirect instances of
\texttt{temporary\_attribute\_object} are not dumped. Coupling
databases with graphical interfaces and \textsc{Gcc} plugins have been
achieved in \cite{Dean:2009:visual}.
