...
\begin{itemize}
  ...
  \item lightweight ``syntax'' oriented ``unsound'' tools, such as
    ...
    warnings and link-time optimization~\footnote{Link-time
      ...
      compiler (e.g. Gimple \index{Gimple} for GCC) is kept in object files and
...
plugins doing inter-process communication) with our \index{persistent
  monitor} \index{persistent!monitor} \emph{persistent monitor},
tentatively called \texttt{bismon}.
...
We show below several examples of optimizations done by recent  \emph{GCC}

\bigskip

{{\raisebox{3pt}{\rule{0.2\textwidth}{1.5pt}}} ~ \large{recursive inlining with constant folding}}
...
where variables are duplicated so that each SSA variable gets assigned
only \emph{once}. The control flow is reduced to \index{basic block}
\emph{basic blocks} (with a single entry point at start, and perhaps
several conditional exit edges). Then special $\phi$ nodes introduce
places where such a variable may come from two other ones (after
branches).
...
{{\raisebox{3pt}{\rule{0.2\textwidth}{1.5pt}}} ~ \large{heap allocation optimization}}

Our second example shows that \emph{GCC} is capable of clever
optimizations around dynamic heap allocation and de-allocation.
