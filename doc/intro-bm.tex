...
\begin{itemize}
  ...
  \item knowing a bit the \index{C11}{C11} standard
...
\begin{itemize}
  ...
  \item \textbf{dynamic typing}, like many scripting languages (such
    ...
    \footnote{\label{fn:initial-gc}The initial GC of the monitor is a crude precise garbage
      ...
      difficult work (past experience on \emph{GCC MELT} taught us
...
persisted as a whole, but tasklets\footnote{Actually tasklets are
  ...
  sending them the predefined selector \texttt{\emph{run\_tasklet}}.}
themselves may be \index{persistent}{persistent} or
\index{transient}{transient}.  Tasklets can also be created outside of
the agenda (e.g. by incoming HTTP requests, by completion of external
processes, by timers, ...)  and added asynchronously into the agenda.

Outside of the agenda, there is a queue of delayed todo closures (a
queue of closures to be run, as if it was an idle priority queue). But
that \index{idle queue}{idle queue} contains (perhaps) a few closures,
not tasklets. Likewise, external running processes have associated to
them the closure to run when they complete.
...
\item the \emph{Bismon monitor} itself, with several threads (notably for the agenda mechanism described above)
  ...
  \item the web browsers of developers using that particular
    \emph{Bismon monitor}; each developer probably runs his/her own
    browser. That web browser is expected to follow latest Web
    technologies and standards (HTML5, Javascript6 i.e. EcmaScript
  ...
  \item \emph{Bismon} should also generate the C++ code of \emph{GCC
    ...
    It is yet unclear how such C++ code for GCC plugins is actually
    compiled and when exactly is it executed. We might generate (at
    least for the first common case of GCC plugins generated for
    developers using \emph{Bismon}) \texttt{*.shar} archives (obtained
    by Web requests) for \emph{GNU sharutils}\footnote{See
...
machines (obviously the web browser is not required to run on the same
machine as the \emph{Bismon monitor}, but even the various
compilations -of code generated by \emph{Bismon}, and the
cross-compilation of IoT code- could happen on other machines).
