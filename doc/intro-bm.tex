...
in that state). A routine or function~\footnote{Technically the
  ...
  is a \eph{Bismon} object reifying the code of that routine.}  is an
  ...
  \item run statement, of class \texttt{basiclo\_run}, which
    ...
    \item for loops, we have a \texttt{basiclo\_while} class of
      ...
      \item failure (inspired by Go's \texttt{panic}) statements are
        ...
        \§ref{subsec:multithreadist} below) running that statement.

      \item locking of objects use a \texttt{basiclo\_lockobj} class
        (mentioning both a object to lock and a sequence of
...
Expressions in our DSL are typed (with types like \textt{value},
...
\item closure application. Often, the closure's connective would be a
  function object.
