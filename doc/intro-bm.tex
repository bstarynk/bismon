...
techniques (most of them being already trained to use \emph{Frama-C}
...
includes formal (but \emph{fully automatic}) proofs of \emph{simple}
...
of Contiki).  In contrast, \textsc{Chariot} focuses mainly on a
\emph{systems of systems} (e.g. networks of systems and systems of
networks) approach, so ``aims to address is how
...
experts, are discovering that their favorite IDE (provided by their
main IoT hardware vendor) is running GCC underneath during the build
of their firmware. No \textsc{Chariot} industrial partner have any
prior extensive experience of static formal-methods based source code
analysis techniques and tools such as \emph{Frama-C}. Most of them
don't even build their firmware on a Linux workstation (but depend on
some proprietary IDE containing an obsolete version of \emph{GCC} and
have to run that on Windows.).
