...
%
      ...
      \cline{2-4}
      & \textbf{ST1.3.3} design and implementation of the persistent monitor & HH & IIxxx III \\
      \cline{2-4}
      & \textbf{ST1.3.4} design and implementation of the compiler and linker extension & §\ref{subsec:compilinkext} & LLyyy LLLL \\
      \hline
%-      \multirow{4}{*}{%
%-     \begin{minipage}[t]{0.18\textwidth}
%-       \smallskip
...
This \textsc{Chariot} deliverable D1.3v1 is the \emph{incomplete
  draft} of a report D1.2v2 scheduled at M30 on \emph{Specialized
  Static Analysis tools for more secure and safer IoT software
  development} and relates to the work performed in \textbf{T1.3}
...
\emph{framework} leveraging on the powerful recent \emph{GCC}
     ...
     then gives the driving principles of our \emph{Bismon} persistent monitor (in §\ref{subsec:principles-bismon}); and explains its multi-threaded and distributed aspects (in §\ref{subsec:multithreadist}). The data and its persistence is detailed (in §\ref{sec:datapersist}, notably §\ref{subsec:dataproc} for the processed data, §\ref{subsec:gcvalobj} for the garbage collection, §\ref{subsec:persistence} for persistence.
...
table \ref{tab:factinlinegimple}. This is a textual (and quite
...
IoT developer will interact with the persistent monitor thru his/her
web \index{browser} browser~\footnote{We don't aim compatibility with all web browsers
...
described here (this incomplete draft D1.3v1 of a future report D1.3v2
scheduled for M30 - in 2020) are not completely implemented in 2018 or
...
\begin{itemize}
  ...
  \item \textbf{homoiconicity} and \textbf{reflection} with
    ...
    \item \textbf{bootstrapped implementation}: \index{bootstrap}
    ...
    \item with \textbf{collaborative web interface}, \index{web interface} used
      ...
      \item with a \textbf{syntax oriented editor} (for our DSLs),
        inspired by the ideas of \textsc{Mentor}
        ...
        1980s), partially shown in a web browser. That web interface
        is facilitating \emph{refactoring} of DSL code.
\end{itemize}
...
and largely experimented in \emph{GCC MELT}) will ease the extension
of that language.
...
strings, etc...). Statements of our DSL include:

\begin{itemize}
\item assign, of class \texttt{basiclo\_assign}, as explained above.
  
  \item run statement, of class \texttt{basiclo\_run}, which
    ...
    \item conditional statement, of class \texttt{basiclo\_cond},
...
screenshot made on october 22\textsuperscript{th} 2018 on git commit
...
\begin{figure}[h]
  ...
  \end{center}
  \caption{crude GTK interface {\relsize{-1}{oct. 22, 2018, git commit \texttt{cbdcf1ec351c3f2a}}}}
...
\item using GTK~\footnote{Since GTK is a free software library, we
  ...
  incompatible with a multi-threaded precise garbage collector, like
  the one in \emph{Bismon} (cf §\ref{subsec:gcvalobj} below), in
  particular because GTK may have several nested event loops, so many
  local garbage collector pointers in internal call frames (which are
  not accessible from routines above).
...
\emph{contributors}- can be voluntarily registered and unregistered on
the command line~\footnote{Use the option \texttt{--contributor=} to
  add them, \texttt{--remove-contributor=} to remove them and
  \texttt{--add-passwords=} to set their encrypted
...
\emph{very high}, \emph{high}, \emph{normal}, \emph{low}, \emph{very
  low}. Each agenda worker thread removes one tasklet (chossing the
