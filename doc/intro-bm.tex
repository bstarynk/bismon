...
\begin{itemize}
  ...
  \item \textbf{dynamic typing}, like many scripting languages (such
    ...
    \footnote{The initial GC of the monitor is a crude precise garbage
  ...
  \item \textbf{homoiconicity} and \textbf{reflection} with
    \textbf{introspection} (cf \cite{Pitrat:2009:ArtifBeings}): all
    the DSL code is explicitly represented as data which can be
    processed by \textit{Bismon}, and the current state is accessible
    by the DSL
    ...
    \item with \textbf{collaborative web interface}, used by a
      ...
      developers\footnote{The initial \texttt{bismon} implementation
        ...
        disappear at end of 2018}. The users of \emph{bismon} are
      expected to trust each other, and to use the \texttt{bismon}
...
persisted as a whole, but tasklets themselves may be
