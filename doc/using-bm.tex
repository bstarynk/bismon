...
Therefore, there is some way to encode any \emph{Bismon} value into a JSON value; this is the canonical JSON encoding of values, given in figure \ref{fig:canon-json} below.
...
\begin{figure}[h]
  \begin{center}
    \begin{tabular}{l c l p{4cm}}
      ...
      {\relsize{-1}{boxed floats encoded as is}} \\
      $\llbracket \textrm{object}~ \omega$ {\relsize{-1.5}{\textrm{of objid}~ \textit{oid}}}$ \rrbracket_{json}$ &
      $\rightarrow$ &
      \texttt{\textbf{\{ "!oid" :}} $\mathit{oid}$ \texttt{\textbf{\}}} &
      {\relsize{-1}{objects encoded with \texttt{"!oid"} JSON attribute giving the \index{objid}objid as a JSON string}} \\
      $\llbracket \textrm{tuple}~ [ \omega_1 \ldots \omega_n ] ~ \rrbracket_{json}$ &
      $\rightarrow$ &
    %  \texttt{\textbf{\{ "!tup" : \[}} $\mathit{oid}_1 \ldots \mathit{oid}_n$  \texttt{\textbf{\]  \}}}
      &
      %- {\relsize{-1}{objects encoded  with \texttt{"!tup"} JSON attribute giving the array of corresponding objid-s % $\mathit{oid}_i = \mathrm{objid} (\omega_i)$
      %-       }}
             \\
    \end{tabular}
    \smallskip
