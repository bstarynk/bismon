  ...
  \begin{center}
    \begin{tabular}{lclp{5.3cm}}
      ...
      \texttt{\textbf{json\_null}}  &
             {\relsize{-1}{The JSON \texttt{null}\index{null!JSON} is decoded as the \texttt{json\_null} Bismon object \texttt{€\_6WOSg1mpN}}}
      \\
      %%%
      $\langle \mathtt{\textit{\texttt{false}}} \rangle^{nod}$ \index{null} &
      $\rightarrow$ &
      \texttt{\textbf{json\_false}}  &
             {\relsize{-1}{The JSON \texttt{false} is decoded as the \texttt{json\_false} Bismon object \texttt{€\_1h1MMlmQi}}}
      \\
      %%%
      $\langle \mathtt{\textit{\texttt{true}}} \rangle^{nod}$ \index{null} &
      $\rightarrow$ &
      \texttt{\textbf{json\_true}}  &
             {\relsize{-1}{The JSON \texttt{true} is decoded as the \texttt{json\_true} Bismon object \texttt{€\_0ekuRPtKaI}}}
      \\
      %%%
      $\langle \mathrm{integer} ~ \iota \rangle^{nod}$  &
      $\rightarrow$ &
      tagged integer $\iota$  &
             {\relsize{-1}{The JSON integers are decoded as the corresponding \emph{Bismon} integer}}
      \\
      %%%
      $\langle \mathrm{real} ~ \delta \rangle^{nod}$  &
      $\rightarrow$ &
      boxed double $\delta$  &
             {\relsize{-1}{The JSON reals are decoded as the corresponding \emph{Bismon} boxed double}}
      \\
      \rule{0pt}{2ex} \\
      %%%
      ...
      $\rightarrow$ &
      object $\omega$  &
      {\relsize{-1}{an objid-looking string $\sigma$ \emph{may} be decoded as
          \emph{the existing} object $\omega$ such as
          $\mathrm{objid}(\omega) = \sigma$}}
      \\
      %%%
      $\langle \mathrm{\textrm{object-naming} ~ string ~} \sigma \rangle^{nod}$ \index{objid} &
      $\rightarrow$ &
      object $\omega$  &
      {\relsize{-1}{a name string $\sigma$ \emph{may} be decoded as
          \emph{the existing} object $\omega$ such as
      ...
      \end{minipage} &
      {\relsize{-1}{a name-looking string $\sigma$, for names used here, is decoded as
      ...
      {\relsize{-1}{a string $\sigma$ \emph{would} otherwise be decoded as
      ...
      \end{minipage}
      &
       {\relsize{-1}{A JSON array is compositionally decoded into a node of connective \texttt{json\_array} and sons given by decoding the components of that array}} \\
      \rule{0pt}{6ex} \\
    \end{tabular}
  \end{center}
