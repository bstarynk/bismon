...
\begin{table}[!htbp]
  ...
  \begin{center}
    \begin{tabular}{lcp{5.3cm}}
      ...
      %%%
      \texttt{\textbf{* json\_null ()}} &
      $js \equiv \mathtt{null}$ &
      succeeds when $js$ is the null JSON \\
      %%%
      \texttt{\textbf{* json\_false ()}} &
      $js \equiv \mathtt{false}$ &
      succeeds when $js$ is the false JSON \\
      %%%
      \texttt{\textbf{* json\_true ()}} &
      $js \equiv \mathtt{true}$ &
      succeeds when $js$ is the true JSON \\
      %%%
      \texttt{\textbf{* int ($v$)}} &
      $js \equiv \mathrm{some ~ JSON ~ integer}~ i$ &
      succeeds when $js$ is a JSON integer $i$, and assign to integer
      variable $v$ its integral value $v \leftarrow i$\\
      %%%
      \texttt{\textbf{* double\_float ($v$)}} &
      $js \equiv \mathrm{some ~ JSON ~ real}~ \delta$ &
      succeeds when $js$ is a JSON real $\delta$, and assign to double
      variable $v$ its \emph{boxed} double value $v \leftarrow
      \mathrm{\textrm{boxed-double}} (\delta)$\\
      %%%
      \texttt{\textbf{* string ($v$)}} &
      $js \equiv \mathrm{some ~ JSON ~ string}~ \sigma$ &
      succeeds when $js$ is a JSON string $\sigma$, and assign to
      ...
      \begin{minipage}{4cm}
        $js \equiv \mathrm{some ~ JSON ~ array}$ \\
        \hspace*{2em}  $ \mathtt{\textbf{[}} js_1  \mathtt{\textbf{,}} \ldots  js_n \mathtt{\textbf{]}}$
      \end{minipage}
      &
      succeeds when $js$ is a JSON array of \emph{\textbf{exactly}}
      $n$ elements $js_1, \ldots js_n$, and extracts them in sequence,
      using $\chi_1$ on $js_1$, then $\chi_2$ on $js_2$, etc... and at
      last $\chi_n$ on $js_n$
      \\
  ...
  \end{center}
  \bigskip
  Notation: $\chi, \chi', \chi_i$... are extractors, $\mathit{js},
  \mathit{js}', \mathit{js}_i$... are JSON values, $v, v', v'', v_i,
  w, $... are \emph{Bismon} local variables whose type is explained.
  \end{relsize}
  \caption{extraction of JSON from some JSON $\mathit{js}$.}
