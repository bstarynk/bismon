...
\item instances of \texttt{html\_void\_element\_object} emit some void
  ...
  selector should emit -as a side effect- the opening tag \texttt{<hr
    class='foo'} without the ending \texttt{/>}. {\color{red}{@@ perhaps \texttt{emit\_xhtml\_open} should return the name of the tag, e.g. \texttt{hr} in this example? If yes, how? @@}}
\item instances of \texttt{html\_element\_object} emit recursively
  ...
  any separators in the content. The start tag is emitted with \texttt{emit\_xhtml\_open} like before, then the components, then the end tag with \texttt{emit\_xhtml\_close}  {\color{red}{@@ not sure, not really needed if \texttt{emit\_xhtml\_open} is giving the tag name...@@}}
