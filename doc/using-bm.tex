...
unsatisfactory (since that GC could ``block'' the \texttt{bismon} for
more than half a second - during which the web interface stays
unresponsive, if running the GC on a large enough heap; but see
footnote \ref{fn:initial-gc} suggesting an improvement).
...
\item for any other object $\omega_{conn}$ as connective, we extract
  ...
  element tag. We also extract its \texttt{spacing\_xhtml}
