...
\textit {extractornode} \texttt{)} readmacro, \index{extract
  json@\texttt{extract\_json}} \index{JSON!extractor}
\index{extractor!from JSON} can extract data from the JSON in a payload (of
some object \textit{objson}, usually of class
\texttt{json\_object}). That extraction is driven by the given
\textit{extractornode}, which should be a node.
