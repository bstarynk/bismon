...
object~\footnote{So that \texttt{emit\_xhtml} is, like PHP, a
  ...
  representation.}. That \texttt{emit\_xhtml}, which get as arguments:
...
HTML-reifying value \texttt{v\_html} is nil, nothing happens. When it
is a scalar, it is emitted trivially: a string is emitted HTML-encoded
(so \texttt{\&lt;} for \texttt{<}, etc...); a tagged integer is
emitted in decimal notation; When \texttt{v\_html} is an object, it is
emitted per the following rules:
...
When  \texttt{v\_html} is a node of connective $\omega_{html}$, it is emitted per the following rules:

\begin{itemize}

\item if $\omega_{html}$ is one of \texttt{int}, \texttt{hexa},
