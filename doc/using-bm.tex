...
\item if $\omega_{conn}$ is \texttt{sequence}, every son of
  ...
  $depth$.  \index{sequence@\texttt{sequence}}

\item if $\omega_{conn}$ is \texttt{space}, every son of
  \texttt{v\_html} is emitted in sequence, with an incremented
  $depth$. Between each son, a space is emitted.
  \index{space@\texttt{space}}

\item if $\omega_{conn}$ is \texttt{newline}, every son of
  \texttt{v\_html} is emitted in sequence, with an incremented
  $depth$. Between each son, a newline is emitted.
  \index{newline@\texttt{newline}}
