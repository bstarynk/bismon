...
against  \index{denial-of-service} denial-of-service attacks. The
network administrator running \emph{bismon} could deploy
usual relevant techniques (firewalls, \texttt{iptables}, HTTP
proxying, DMZ, etc ...) to avoid them. The browsers instances
interacting with \emph{bismon} are few (only a dozen of people in some
IoT development team, and each uses his/her graphical browser - a
\emph{recent} Firefox or Chrome~\footnote{So no particular effort is
  even taken to support a variety of old browsers: we don't have any
  code to e.g. support Internet Explorer pecularities or
...
connections or of web sessions is tiny. Therefore most web optimizations
are practically unneeded.
...
of class \texttt{webexchange\_object}. Their payload contains not only
a string buffer, to be filled with the HTTP response content, but also
mentions the web request (as processed by \texttt{libonion}) and the
web session object computed from the \texttt{BISMONCOOKIE} and may
contain some arbitrary data value. That web session object is created
at web login time and is of class \texttt{websession\_object}. It
knows the contributor who is logged in, the expiration time of the
session, some session data (an arbitrary \emph{bismon} value), and the
web socket \index{web socket} connection (if any) to the browser using
that session. An inactive web session expires in about an
...
string and to the web exchange object in the
...
\texttt{webdict\_root}~\footnote{For example: an URL like
  ...
  tasklets into the agenda, etc...- are useful.}  predefined
object. If no web handler closure is found, an \texttt{404 Not found}
status is returned. The \texttt{the\_web\_sessions} predefined object
stores the dictionnary of transient web session objects and associates
a cookie string to its web session object. That dictionnary is
forcibly cleared at start of the web server inside \emph{bismon},
but it should be loaded empty, since web session objects are created
and should remain transient.
...
object. That \texttt{emit\_xhtml}, which get as arguments:
  ...
  \texttt{v\_depth} (which is in general \emph{not} the emitted
  ...
  object, it is emitted per the following rules:
...
\item instances of \texttt{html\_void\_element\_object} emit some void XHTML5 element like e.g. \texttt{<hr />}.
\item instances of \texttt{html\_element\_object} emit recursively
    some nested XHTML element starting with a start tag like
    \texttt{<div} but ending with an end tag like \texttt{</div>}; the
    components of that objects are emitted recursively (with an
    incremented recursion depth).
...
\item any other object is emitted by its name, if it has some, or by
  its objid. This is mostly intended to represent common names or
