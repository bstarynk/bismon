...
const GOptionEntry optionstab_bm[] = {
  ...
  {.long_name = " pid - file ",.short_name = (char) 0,
   ...
   " \ t ... default is _bismon.pid; \n "
   " \ t (if the pid file exists and does not start with a dot or percent, \n "
   " \ t and mentions a valid pid, its process, \n "
   " \ t presumably some other Bismon process, \n "
   " \ t gets a SIGQUIT signal, and has one second to dump its state) ",
   .arg_description = " PATH "},
  //
  ...
  {.long_name = " init - after - load ",.short_name = 'i',
   ...
   .description = "
   do
the < closure > after loading, as initialization ",
  ...
  {.long_name = " test - plugin ",.short_name = (char) 0,
   ...
   .description =
   " run the drafts / testplugin_PLUGINAME.so (after loading) the \ n "
   " \ t ... test - plugin PLUGINAME ",
  ...
  {.long_name = " contributor ",.short_name = (char) 0,
   ...
   " \ t like 'First Lastname <email@example.com>' \ n "
   " \ t or 'First Lastname;email@example.com;aliasmail@example.org' \ n "
   " \ t (this puts personal information relevant to European GDPR in file "
  ...
  {.long_name = " print - contributor - of - oid ",.short_name = (char) 0,
   ...
   .description =
   " print tab - separated: (full name, objid, email, alias) of contributor \ n "
   " \ t ... of given CONTRIBOID then exit, ",
  ...
  {.long_name = " remove - contributor ",.short_name = (char) 0,
   ...
   " \ t like 'First Lastname' \ n "
   " \ t or email @ example.com \ n "
   " \ t or some existing contributor oid similar to _2PFRochKb3N_3e8RFFAUi9K \ n "
   " \ t (this should remove personal information relevant to European GDPR in file "
   CONTRIBUTORS_FILE_BM ") ",
   .arg_description = " CONTRIBUTOR "},
  ///
  {.long_name = " cleanup ",      //
   ...
   .description =
   " cleanup memory at end (for valgrind) \ n "
   " \ t ... (see valgrind.org for more).",
  ...
  {.long_name = " ssl - certificate ",.short_name = (char) 0,
   ...
   .description =
   " Uses FILEPREFIX.pem & FILEPREFIX.key for SSL certificate to libonion \ n "
   " \ t web services.Could be used by GCC plugins ",
  ...
  {.long_name = " web - base ",.short_name = (char) 0,
   ...
   .description =
   " A string like < host >: <port >, default (also when given as _) \ n "
   " is localhost:
8086, describing the base of web URLs served by bismon ",
  ...
  ///
  {
   ...
   .description =
   " Creates / uses / listen a unix (7) socket named UNIX_SOCKET on the local machine. \ n "
   " Input and output protocol is JSON, each message ended by double newlines or a single formfeed. \ n "
  ...
  {.long_name = " bismon - contributors - file ",.short_name = (char) 0,
   ...
   .arg_data = &contributors_filepath_BM,
   .description = " use PATH as the contributors file; \n "
   " \ t..default is contributors_BM or $HOME / contributors_BM ",
   .arg_description = " PATH "},
  //
  {.long_name = " bismon - passwords - file ",.short_name = (char) 0,
   ...
   .arg_data = &passwords_filepath_BM,
   .description = " use PATH as the password file; \n "
   " \ t..default is passwords_BM or $HOME / passwords_BM ",
   .arg_description = " PATH "},
  //
  {.long_name = " bismon - contact - file ",.short_name = (char) 0,
   ...
   .arg_data = &contact_filepath_BM,
   .description = " use PATH as the master contact file; \n "
   " \ t..default is contact_BM or $HOME / contact_BM ",
   .arg_description = " PATH "},
  //
  {.long_name = " bismon - sigusr1 - dump - prefix ",.short_name = (char) 0,
   ...
   .arg_data = &sigusr1_dump_prefix_BM,
   .description =
   " if given a directory prefix DUMP_PREFIX, every SIGUSR1 signal \ n "
   " \ t ... dumps the state into it.For \ n "
   " \ t ... example-- bismon - sigusr1 - dump - prefix = /tmp / dumpBM will \ n "
   " \ t ... make the first dump into / tmp / dumpBM1 /, then \ n "
   " \ t ... the second dump into / tmp / dumpBM2 / etc ... \n ",
   .arg_description = " DUMP_PREFIX "},
  //
  {.long_name = " bismon - add - passwords ",.short_name = (char) 0,
   ...
   .arg_data = &added_passwords_filepath_BM,
   .description =
   " use the given PASSWORDENTRIES file (if it is -, stdin) containing \ n "
   " \ t ... lines like < username >:
   <password > to add passwords ",
   .arg_description = " PASSWORDENTRIES "},
  //
  //
  ...
  //
  {.long_name = " bismon - sleep - delay ",.short_name = (char) 0,
   .flags = G_OPTION_FLAG_NONE,
   .arg = G_OPTION_ARG_DOUBLE,
   .arg_data = &sleepdelay_bm,
   .description = " sleep the given DELAY (in seconds) at start, after load ",
  ...
  {.long_name = " bismon - pid - file ",.short_name = (char) 0,
   ...
   " \ t..default is _bismon.pid. \ n "
   " \ t (if the pid file exists and mentions a valid pid, its process \ n "
   " \ t gets a SIGQUIT signal, and has one second to dump its state) \ n ",
   .arg_description = " PATH "},
  //
...
the < closure > after loading, as initialization ",
  ...
  {.long_name = " bismon - print - contributor - of - oid ",.short_name = (char) 0,
   ...
   .description =
   " print tab - separated:
(full name, objid, email, alias) of contributor \ n "
   " \ t ... of given CONTRIBOID then exit, ",
   .arg_description = " CONTRIBOID "},
  ...
  {.long_name = " bismon - remove - contributor ",.short_name = (char) 0,
   ...
   .arg_data = &remove_contributor_bm,
   .description = " remove existing contributor CONTRIBUTOR, \n "
   " \ t ... like 'First Lastname' or email @ example.com \ n "
   " \ t ... or some existing contributor oid similar to e.g. \ n "
   " \ t ... _2PFRochKb3N_3e8RFFAUi9K (this should remove personal \ n "
   " \ t ... information relevant to European GDPR in file "
   CONTRIBUTORS_FILE_BM ") ",
   .arg_description = " CONTRIBUTOR "},
  ///
  ...
  {.long_name = " bismon - anon - web - cookie ",.short_name = (char) 0,
   ...
   .arg_data = &onion_anon_web_cookie_BM,
   .description =
   " Create an anonymous web HTTP cookie, and write its cookie in \ n "
  ...
  {.long_name = " bismon - unix - json - socket ",.short_name = (char) 0,
   ...
   .arg_data = &unix_json_socket_BM,
   .description =
   " Create / uses / listen a unix (7) socket named UNIX_SOCKET on the local \ n "
...
#define HAS_DELIM_BM(Str,Name) do {			\
    ...
    if (strcmp(Str,prevdelim)<=0)			\
      FATAL_BM(" unsorted delimiter
#%d '%s' and '%s'",	\
	       delimcnt, Str, prevdelim);		\
    prevdelim = Str;					\
  } while(0);
#include "bm_delim.h"
   if (delimcnt != BM_NB_DELIM) FATAL_BM ("expected %d delimiters, got %d", BM_NB_DELIM, delimcnt);}    /* end check_delims_BM */

   void add_new_predefined_bm (void)
   {
   ...
   const objectval_tyBM * predobj = findnamedobj_BM (predname); if (!predobj)
   {
   predobj = makeobj_BM (); registername_BM (predobj, predname);}
   else
   {
   char idpred[32];
   ...
   static int idqcmp_BM (const void *p1, const void *p2)
   {
   return cmpid_BM (*(rawid_tyBM *) p1, *(rawid_tyBM *) p2);}   /* end idqcmp_BM */
   ...
   void do_emit_module_from_main_BM (void)
   {
   LOCALFRAME_BM (NULL, /*descr: */ BMP_emit_module,
      ...
      ); WEAKASSERTRET_BM (module_to_emit_bm != NULL); _.failres = NULL; _.failplace = NULL; int failcod = 0; struct failurelockset_stBM flockset = { }; struct failurehandler_stBM *prevfailurehandle = (struct failurehandler_stBM *) curfailurehandle_BM; initialize_failurelockset_BM (&flockset, sizeof (flockset)); LOCAL_FAILURE_HANDLE_BM (&flockset, lab_failureemit, failcod, _.failres, _.failplace); if (failcod > 0) lab_failureemit:{
   ...
   ("Failed to emit module from main %s, with failcode#%d, failres %s\n"
    "failplace %s", objectdbg_BM (_.modulob), failcod,
    OUTSTRVALUE_BM (_.failres), OUTSTRVALUE_BM (_.failplace)); return;};
   };
   INFOPRINTF_BM ("begin emit module from main: %s\n", module_to_emit_bm); _.parsob = makeobj_BM (); bool gotobj = false; struct parser_stBM *pars = makeparser_memopen_BM (module_to_emit_bm, strlen (module_to_emit_bm), _.parsob); _.modulob = parsergetobject_BM (pars, CURFRAME_BM, 0, &gotobj); DBGPRINTF_BM ("do_emit_module_from_main_BM modulob=%s parsob=%s", objectdbg_BM (_.modulob), objectdbg1_BM (_.parsob)); objlock_BM (_.modulob); _.resultv = send0_BM (_.modulob, BMP_emit_module, CURFRAME_BM); objunlock_BM (_.modulob); objclearpayload_BM (_.parsob); destroy_failurelockset_BM (&flockset); curfailurehandle_BM = prevfailurehandle; DBGPRINTF_BM ("do_emit_module_from_main_bm end modulob=%s result %s",     //
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            objectdbg_BM (_.modulob),   //
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            debug_outstr_value_BM
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            (_.resultv,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             CURFRAME_BM,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             0));
   if (_.resultv) INFOPRINTF_BM ("successful emit module from main: %s\n",
                                 module_to_emit_bm);
   else
   FATAL_BM ("failed emit module from main: %s", module_to_emit_bm); char modulidbuf[32]; memset (modulidbuf, 0, sizeof (modulidbuf)); idtocbuf32_BM (objid_BM (_.modulob), modulidbuf); char makemodulecmd[128]; memset (makemodulecmd, 0, sizeof (makemodulecmd)); snprintf (makemodulecmd, sizeof (makemodulecmd), "%s/build-bismon-module.sh %s", bismon_directory, modulidbuf); DBGPRINTF_BM ("do_emit_module_from_main_bm makemodulecmd=%s", makemodulecmd); fflush (NULL); int cmdcod = system (makemodulecmd); if (cmdcod) FATAL_BM ("failed module making %s (%d)", makemodulecmd, cmdcod); INFOPRINTF_BM ("successfully compiled emitted module %s (%s) from main\n", module_to_emit_bm, modulidbuf); fflush (NULL); return;} /* end do_emit_module_from_main_BM */
   ...
   static void check_locale_BM (void); void check_locale_BM (void)
   {
   ...
   if (!setlocale (LC_ALL, "C.UTF-8") && !setlocale (LC_ALL, "C.utf8") && !setlocale (LC_ALL, "POSIX") && !setlocale (LC_ALL, "POSIX.utf8") && !setlocale (LC_ALL, "POSIX.utf-8") && !setlocale (LC_ALL, "en_US.utf8") && !setlocale (LC_ALL, "en_US.utf-8") && !setlocale (LC_ALL, "en_GB.utf8") && !setlocale (LC_ALL, "en_GB.utf-8")) FATAL_BM ("failed to setlocale LC_ALL appropriately to English UTF-8, previous was %s", oldloc ? : "*unset*"); DBGPRINTF_BM ("now LC_ALL locale is %s", setlocale (LC_ALL, NULL)); double x = 0; int pos = 0; if (sscanf ("4.5;", "%lf%n", &x, &pos) < 1 || x != 4.5 || pos != 3) FATAL_BM ("something wrong (probably your locale setting, which should be C.UTF-8)." " Since '4.5;' is scanned as %f at position#%d", x, pos); DBGPRINTF_BM ("after sscanf x=%f, pos#%d", x, pos); char *end = NULL; x = strtod ("4.5/", &end); DBGPRINTF_BM ("after strtod x=%f, end=%s", x, end); if (x != 4.5 || !end || *end != '/') FATAL_BM ("something wrong (probably your locale setting, which should be C.UTF-8)." " Since '4.5/' is not converted as %f end at %s", x, end); x = 0.0; if (sscanf ("-6.022e23;", "%lf%n", &x, &pos) < 1 || pos != 9 || fabs (x + 6.022e+23) > 1e20) FATAL_BM ("something wrong (probably your locale setting, which should be C.UTF-8)." " Since '-6.022e23;' is scanned as %g at position#%d", x, pos);} /* end of check_locale_BM */
...
/////
   ...
   {
   if (!out) out = stderr; for (int aix = 0; aix < argc; aix++)
   {
   if (aix > 0)
   fputc (' ', out);
   if (argv[aix] == NULL) fprintf (out, "#?arg%d_null?#", aix);
   else
   {
   gchar * qa = g_shell_quote ((const gchar *) argv[aix]);
   fputs ((const char *) qa, out); g_free (qa);}
   }
...
//// GTK appplications
   ...
   {
   clock_gettime (CLOCK_MONOTONIC, &startrealtimespec_BM); if (argc <= 0)
   ...
   if (!strncmp
       (argv[ix], "--print-contributor-of-oid",
        strlen ("--print-contributor-of-oid"))) skiplocalcheck = true;}
   ///
   {
   ...
   parse_program_options_BM (argc, argv); if (randomseed_BM > 0)
   {
   ...
   initialize_predefined_names_BM (); initialize_agenda_BM ();
   // GError *opterr = NULL;
   ...
   test_make_empty_sigusr1_dump_dir_BM (); if (print_contributor_of_oid_bm)
   {
   ...
   if (nbcores < 5 * MAXNBWORKJOBS_BM) nbworkjobs_BM = nbcores / 5 + 1;
   else
   nbworkjobs_BM = MAXNBWORKJOBS_BM;}
   if (nbworkjobs_BM < MINNBWORKJOBS_BM) nbworkjobs_BM = MINNBWORKJOBS_BM + 1;
   else
   if (nbworkjobs_BM > MAXNBWORKJOBS_BM)
   nbworkjobs_BM = MAXNBWORKJOBS_BM;
   ...
   initialize_contact_path_BM (); parse_contact_BM ();
   //
   if (count_emit_has_predef_bm > 0)
   emit_has_predef_BM ();
   DBGPRINTF_BM ("run_onion is %s", run_onion_BM ? "true" : "false");
   if (!load_dir_bm) load_dir_bm = ".";
   else
   if (load_dir_bm[0] == '-')
   {
   ...
   if (!dump_dir_BM) dump_dir_BM = realpath (load_dir_bm, NULL);
   else
   if (dump_dir_BM[0] == '-')
   {
   ...
   INFOPRINTF_BM ("changed directory, given as %s, after load\n",
                  chdir_after_load_bm); fflush (NULL);}
   if (debug_after_load_BM)
   {
   INFOPRINTF_BM ("bismon with debug messages after load from %s",
                  load_dir_bm); showdebugmsg_BM = true;};
   if (sleepdelay_bm > 0.0)
   {
   DBGPRINTF_BM ("sleepdelay_bm %g", sleepdelay_bm);
   if (sleepdelay_bm > 1000.0)
   sleepdelay_bm = 1000;
   if (sleepdelay_bm > 2.0)
   INFOPRINTF_BM ("will sleep for %g seconds", sleepdelay_bm);
   int delayms = (int) (sleepdelay_bm * 1000.0);
   usleep (delayms);
   INFOPRINTF_BM ("slept for %g seconds\n", sleepdelay_bm);};
   show_net_info_bm (); if (!batch_bm)
   {
   if (run_onion_BM)
   {
   INFOPRINTF_BM
   ("Bismon pid %d on %s (git %s) initializing ONION for Web services",
   ...
   fflush (NULL);
   if (count_init_afterload_bm > 0)
   init_afterload_bm (); if (dump_after_load_dir_bm)
   {
   if (debug_after_load_BM)
   {
   INFOPRINTF_BM
   ("Bismon (pid %d on %s git %s) disabling debug before dump after load to %s",
    (int) getpid (), myhostname_BM, bismon_shortgitid,
    dump_after_load_dir_bm); showdebugmsg_BM = false;}
   ...
   if (batch_bm)
   {
   nbworkjobs_BM = 0; INFOPRINTF_BM ("no web in batch mode\n");}
   else
   {
   {
   char cwdbuf[128];
...
#warning should have an event loop for Unix json socket
  ...
  //
   ...
   usleep (200); full_garbage_collection_BM (NULL);}
   DBGPRINTF_BM ("ending BISMON run_onion_BM %s batch_bm %s", run_onion_BM ? "true" : "false", batch_bm ? "true" : "false"); free ((void *) contributors_filepath_BM), contributors_filepath_BM = NULL; free ((void *) passwords_filepath_BM), passwords_filepath_BM = NULL; if (dump_dir_BM) free ((void *) dump_dir_BM), dump_dir_BM = NULL; if (want_cleanup_bm) final_cleanup_BM (); INFOPRINTF_BM ("end of %s, on %s, pid %d, %.3f elapsed, %.3f cpu time\n" "... timestamp %s\n" "... lastgitcommit %s\n" "... checksum %s\n", myprogname_BM, myhostname_BM, (int) getpid (), elapsedtime_BM (), cputime_BM (), bismon_timestamp, bismon_lastgitcommit, bismon_checksum); fflush (NULL);} /* end main */


   bool is_nice_locale_BM (const char *locstr)
   {
#define NICE_LOCALE_REGEX_BM "^\\(en_[a-z_]*\\|C\\|POSIX\\)\\.*utf.*8.*"
   static regex_t rx; static bool initedrx;
   /// initialize rx only once
   if (!initedrx)
   {
   int errcod =
   regcomp (&rx, NICE_LOCALE_REGEX_BM, REG_EXTENDED | REG_ICASE); if (errcod)
   {                            // should never happen
   char errbuf[128];
   ...
   if (regexec (&rx, locstr, 1, rm, 0)) return false;
   else
   return true;}                /* end is_nice_locale_BM */



   bool bismon_has_web_BM (void)
   {
   return web_is_running_BM || run_onion_BM;}   /* end bismon_has_web_BM */

   void do_dump_after_load_BM (void)
   {
   {
   char *rd = realpath (dump_after_load_dir_bm, NULL); if (rd)
   {
   INFOPRINTF_BM ("dump after load into %s directory (%s)",
                  dump_after_load_dir_bm, rd); free (rd), rd = NULL;}
   else
   INFOPRINTF_BM ("dump after load into %s directory",
                  dump_after_load_dir_bm);}
   struct dumpinfo_stBM di = dump_BM (dump_after_load_dir_bm, NULL); INFOPRINTF_BM ("dump after load: scanned %ld, emitted %ld objects\n" "... did %ld todos, wrote %ld files\n" "... in %.3f elapsed, %.4f cpu seconds\n (%.1f elapsed, %.1f cpu µs/obj)\n", di.dumpinfo_scanedobjectcount, di.dumpinfo_emittedobjectcount, di.dumpinfo_todocount, di.dumpinfo_wrotefilecount, di.dumpinfo_elapsedtime, di.dumpinfo_cputime, di.dumpinfo_elapsedtime * 1.0e6 / di.dumpinfo_emittedobjectcount, di.dumpinfo_cputime * 1.0e6 / di.dumpinfo_emittedobjectcount);}        /*  end do_dump_after_load_BM */



   void emit_has_predef_BM (void)
   {
   rawid_tyBM * idarr =
   calloc (count_emit_has_predef_bm, sizeof (rawid_tyBM));
   if (!idarr) FATAL_BM ("failed to calloc idarr for %d",
                         count_emit_has_predef_bm); for (int ix = 0;
                                                         ix <
   ...
   char idbuf[32];
   memset (idbuf, 0, sizeof (idbuf));
   idtocbuf32_BM (id, idbuf); printf (" ROUTINEOBJNAME_BM (%s)\n", idbuf);}
   printf ("***/\n\n\n"); INFOPRINTF_BM ("/// emitted %d extra predefs\n", count_emit_has_predef_bm);}  /* end emit_has_predef_BM */


   void initialize_contributors_path_BM (void)
   {
   ...
   if ((homepath = bismon_home_BM ()) != NULL
       && asprintf (&path, "%s/" CONTRIBUTORS_FILE_BM, homepath) > 0
       && path != NULL && (!access (path, R_OK)
                           || (free (path), (path = NULL))))
   contributors_filepath_BM = path; if (!contributors_filepath_BM)
   {
   char cwdbuf[128];
   memset (cwdbuf, 0, sizeof (cwdbuf));
   ...
   FATAL_BM ("cannot get real path of contributors file %s - %m",
             contributors_filepath_BM);
   if (path && rcpath != path)
   free (path), path = NULL; contributors_filepath_BM = rcpath;}
   else
   ...
   if (!rcpath)
   FATAL_BM ("cannot get real path of contributors file %s - %m",
             contributors_filepath_BM); contributors_filepath_BM = rcpath;}
   if (access (contributors_filepath_BM, R_OK)) FATAL_BM ("cannot read real contributors file %s - %m", contributors_filepath_BM); if (!print_contributor_of_oid_bm)    // otherwise silence
   INFOPRINTF_BM ("using %s as the contributors file", contributors_filepath_BM);}      /* end initialize_contributors_path_BM */



////////////////

   void initialize_passwords_path_BM (void)
   {
   ...
   if ((homepath = bismon_home_BM ()) != NULL
       ...
       && path != NULL && (!access (path, R_OK)
                           || (free (path), (path = NULL))))
   passwords_filepath_BM = path; if (!passwords_filepath_BM)
   {
   char cwdbuf[128];
   memset (cwdbuf, 0, sizeof (cwdbuf));
   if (!getcwd (cwdbuf, sizeof (cwdbuf)))
   ...
   FATAL_BM ("cannot get real path of passwords file %s - %m",
             passwords_filepath_BM);
   if (path && rcpath != path)
   free (path), path = NULL; passwords_filepath_BM = rcpath;}
   else
   ...
   if (!rcpath)
   FATAL_BM ("cannot get real path of passwords file %s - %m",
             passwords_filepath_BM); passwords_filepath_BM = rcpath;}
   if (access (passwords_filepath_BM, R_OK)) FATAL_BM ("cannot read real passwords file %s - %m", passwords_filepath_BM); struct stat mystat = { }; if (stat (passwords_filepath_BM, &mystat)) FATAL_BM ("cannot stat real passwords file %s - %m", passwords_filepath_BM); if ((mystat.st_mode & S_IFMT) != S_IFREG) FATAL_BM ("real passwords file %s is not a regular file", passwords_filepath_BM); if ((mystat.st_mode & (S_IRWXG | S_IRWXO)) != 0) FATAL_BM ("real passwords file %s should not be group or others readable/writable but only by owner; run chmod go-rwx on it", passwords_filepath_BM); INFOPRINTF_BM ("using %s as the password file", passwords_filepath_BM);} /* end initialize_passwords_path_BM */


   void initialize_contact_path_BM (void)
   {
   if (!contact_filepath_BM)    // no --passwords-file program option
   {
   char *path = NULL;
   const char *homepath = NULL;
   if (!access (CONTACT_FILE_BM, R_OK)) contact_filepath_BM = CONTACT_FILE_BM;
   else
   if ((homepath = bismon_home_BM ()) != NULL
       && asprintf (&path, "%s/" CONTACT_FILE_BM, homepath) > 0
       && path != NULL && (!access (path, R_OK)
                           || (free (path), (path = NULL))))
   contact_filepath_BM = path; if (!contact_filepath_BM)
   {
   char cwdbuf[128];
   memset (cwdbuf, 0, sizeof (cwdbuf));
   if (!getcwd (cwdbuf, sizeof (cwdbuf)))
   ...
   FATAL_BM ("cannot get real path of contact file %s - %m",
             contact_filepath_BM);
   if (path && rcpath != path)
   free (path), path = NULL; contact_filepath_BM = rcpath;}
   else
   ...
   if (!rcpath)
   FATAL_BM ("cannot get real path of contact file %s - %m",
             contact_filepath_BM); contact_filepath_BM = rcpath;}
   if (access (contact_filepath_BM, R_OK)) FATAL_BM ("cannot read real contact file %s - %m", contact_filepath_BM); struct stat mystat = { }; if (stat (contact_filepath_BM, &mystat)) FATAL_BM ("cannot stat real contact file %s - %m", contact_filepath_BM); if ((mystat.st_mode & S_IFMT) != S_IFREG) FATAL_BM ("real contact file %s is not a regular file", contact_filepath_BM); if ((mystat.st_mode & (S_IRWXG | S_IRWXO)) != 0) FATAL_BM ("real contact file %s should not be group or others readable/writable but only by owner; run chmod go-rwx on it", contact_filepath_BM); INFOPRINTF_BM ("using %s as the contact file", contact_filepath_BM);}        /* end initialize_contact_path_BM */

   void parse_contact_BM (void)
   {
...
////////////////
   void parse_values_after_load_BM (void)
   {
...
////////////////
   ...
   _.parsob = makeobj_BM ();
   struct parser_stBM *pars =
   makeparser_of_file_BM (pfil, filepath, _.parsob); int parsvalcnt = 0;
   bool gotval = false;
   do
   {
   gotval = false;
   ...
   INFOPRINTF_BM ("parsed value#%d from %s is:\n%s\n", parsvalcnt,
                  filepath, debug_outstr_value_BM (_.parsedval,
                                                   CURFRAME_BM, 0));}
   fflush (NULL);}
   while (gotval); fclose (pfil); objclearpayload_BM (_.parsob); INFOPRINTF_BM ("done parsing %d values from %s after load\n", parsvalcnt, filepath);}  /* end parse_a_file_after_load_BM */


   void parse_files_after_load_BM (void)
   {
   ...
   INFOPRINTF_BM ("done parsing %d files (pid %d on %s git %s)\n", nb_parsed_files_after_load_bm, (int) getpid (), myhostname_BM, bismon_shortgitid);}  /* end parse_files_after_load_BM */


   void run_testplugins_after_load_BM (void)
   {
...
#define TESTPLUGIN_MAXNAMELEN_BM 40
   ...
   (testplugininit_sigBM *) dlsym (testpluginhdl, initestname); if (inifun)
   {
   DBGPRINTF_BM
   ("run_testplugins_after_load initestname %s before inifun @%p #%d",
    initestname, inifun, ix); (*inifun) (CURFRAME_BM, curtestplugin, ix);
   DBGPRINTF_BM
   ...
   fflush (NULL); for (int ix = 0; ix < nb_testplugins_after_load_bm; ix++)
   {
   char *curtestplugin = testplugins_after_loadarr_bm[ix];
   ...
   INFOPRINTF_BM ("testplugin %s runtestname %s successful (ix#%d)\n", curtestplugin, runtestname, ix); fflush (NULL);} /* end for ix, running */
   double endelapsedtime = elapsedtime_BM (); double endcputime = cputime_BM (); INFOPRINTF_BM ("run_testplugins_after_load_BM done for %d testplugins, %.3f elapsed, %.3f cpu seconds\n", nb_testplugins_after_load_bm, endelapsedtime - startelapsedtime, endcputime - startcputime);}        /* end of run_testplugins_after_load_BM */

////////////////
/// run initializations, if any, after the load
   void init_afterload_bm ()
   {
   LOCALFRAME_BM ( /*prev stackf: */ NULL, /*descr: */ NULL,
   ...
   WARNPRINTF_BM ("bad do-after-load non-closure#%d:\n%s\n", ix,
                  OUTSTRVALUE_BM (_.parsedval));
   arr_init_afterload_bm[ix] = NULL; free ((void *) curclostr);}
   free (arr_init_afterload_bm), arr_init_afterload_bm = NULL; size_init_afterload_bm = 0;}     /* end init_afterload_bm */



   void add_contributors_after_load_BM (void)
   {
   LOCALFRAME_BM ( /*prev stackf: */ NULL, /*descr: */ NULL,
   ...
   for (int cix = 0; cix < count_added_contributors_bm; cix++) free (added_contributors_arr_bm[cix]), added_contributors_arr_bm[cix] = NULL; INFOPRINTF_BM ("added %d contributors after load", count_added_contributors_bm); free (added_contributors_arr_bm); count_added_contributors_bm = 0; size_added_contributors_bm = 0;}       /* end add_contributors_after_load_BM */




   void remove_contributors_after_load_BM (void)
   {
   LOCALFRAME_BM ( /*prev stackf: */ NULL, /*descr: */ NULL,
   ...
   for (int cix = 0; cix < count_removed_contributors_bm; cix++) free (removed_contributors_arr_bm[cix]), removed_contributors_arr_bm[cix] = NULL; INFOPRINTF_BM ("removed %d contributors after load", count_removed_contributors_bm); free (removed_contributors_arr_bm); count_removed_contributors_bm = 0; size_removed_contributors_bm = 0;}       /* end remove_contributors_after_load_BM */




   void add_passwords_from_file_BM (const char *addedpasspath)
   {
   LOCALFRAME_BM ( /*prev stackf: */ NULL, /*descr: */ NULL,
   ...
   int lincnt = 0;
   int nbch = 0;
   if (!linpas) FATAL_BM ("calloc failure in add_passwords_from_file - %m");
   do
   {
   ...
   *pcol = (char) 0;
   ...
   if (withstdin)
   INFOPRINTF_BM ("Added or changed %d passwords using standard input", nbch);
   else
   INFOPRINTF_BM ("Added or changed %d passwords using file %s", nbch, addedpasspath);} /* end of add_passwords_from_file_BM */
...
// called from did_deferred_BM
   ...
   LOCALFRAME_BM ( /*prev stackf: */ NULL, /*descr: */ NULL,
                  ...
                  value_tyBM failres;   //
                  value_tyBM failplace; //
          ); _.funv = fun; _.arg1v = arg1; _.arg2v = arg2; _.arg3v = arg3; int failcod = 0; _.failres = NULL; _.failplace = NULL; struct failurelockset_stBM flockset = { }; initialize_failurelockset_BM (&flockset, sizeof (flockset)); LOCAL_FAILURE_HANDLE_BM (&flockset, lab_failureapply, failcod, _.failres, _.failplace); if (failcod) lab_failureapply:
   {
   destroy_failurelockset_BM (&flockset); curfailurehandle_BM = NULL; WARNPRINTF_BM ("deffered_apply3_gtk failure, failcod#%d failreason: %s\n", failcod,       //
   ...
   if (isobject_BM (_.funv))
   {
   _.funob = objectcast_BM (_.funv);}
   else
   {
   _.funob = NULL; WARNPRINTF_BM ("internaldeferapply bad funv %s arg1 %s arg2 %s arg3 %s",     //
                                  OUTSTRVALUE_BM (_.funv),      //
                                  OUTSTRVALUE_BM (_.arg1v),     //
                                  OUTSTRVALUE_BM (_.arg2v),     //
                                  OUTSTRVALUE_BM (_.arg3v));}
   _.resappv = NULL; if (!isobject_BM (_.funob) || !_.funob->ob_rout) WARNPRINTF_BM ("internaldeferapply no routine for funv %s arg1 %s arg2 %s arg3 %s",       //
                                                                                     OUTSTRVALUE_BM (_.funv),   //
                                                                                     OUTSTRVALUE_BM (_.arg1v),  //
                                                                                     OUTSTRVALUE_BM (_.arg2v),  //
                                                                                     OUTSTRVALUE_BM
   ...
   DBGPRINTF_BM ("internaldeferapply applied funv %s arg1 %s arg2 %s arg3 %s => resapp %s",     //
                 ...
                 OUTSTRVALUE_BM (_.arg2v),      //
                 OUTSTRVALUE_BM (_.arg3v),      //
                 OUTSTRVALUE_BM (_.resappv)); if (!_.resappv)
   {
   WARNPRINTF_BM ("internaldeferapply NULL result of application of funv %s arg1 %s arg2 %s arg3 %s",   //
                  ...
                  OUTSTRVALUE_BM (_.arg2v),     //
                  OUTSTRVALUE_BM (_.arg3v));}
   destroy_failurelockset_BM (&flockset); curfailurehandle_BM = NULL;}  /* end do_internal_defer_apply3_BM */
...
// called from did_deferred_BM
   ...
   LOCALFRAME_BM ( /*prev stackf: */ NULL, /*descr: */ NULL,
                  ...
                  value_tyBM failplace; //
          ); _.recva = recv; _.obsel = obsel; _.arg1v = arg1; _.arg2v = arg2; _.arg3v = arg3; int failcod = 0; _.failres = NULL; _.failplace = NULL; struct failurelockset_stBM flockset = { }; initialize_failurelockset_BM (&flockset, sizeof (flockset)); LOCAL_FAILURE_HANDLE_BM (&flockset, lab_failuresend, failcod, _.failres, _.failplace); if (failcod) lab_failuresend:
   {
   destroy_failurelockset_BM (&flockset); curfailurehandle_BM = NULL; WARNPRINTF_BM ("deffered_send3_gtk failure, failcod#%d\n" "failreason: %s\n" "failplace: %s\n",   //
                                                                                     failcod,
                                                                                     OUTSTRVALUE_BM
                                                                                     (_.failres),
                                                                                     OUTSTRVALUE_BM
   ...
   DBGPRINTF_BM ("internaldefersend recv %s obsel %s arg1 %s arg2 %s arg3 %s",  //
                 ...
                 debug_outstr_value_BM (_.arg3v,        //
                                        CURFRAME_BM, 0)); (void) send3_BM (recv, obsel, CURFRAME_BM, arg1, arg2, arg3); DBGPRINTF_BM ("internaldefersend did send recv %s obsel %s",    //
                                                                                                                                      debug_outstr_value_BM (_.recva,   //
                                                                                                                                                             CURFRAME_BM, 0),   //
                                                                                                                                      objectdbg_BM (_.obsel)); destroy_failurelockset_BM (&flockset); curfailurehandle_BM = NULL;}      /* end do_internal_defer_send3_BM */



////////////////////////////////////////////////////////////////
/// mail testing
   void do_test_mailhtml_bm (void)
   {
   ...
   if (valid_contributor_name_BM (mailhtml_contributor_bm, &errmsg))
   {
   _.contribob =
   ...
   DBGPRINTF_BM ("do_test_mailhtml_bm contributor='%s' == %s",
                 mailhtml_contributor_bm, objectdbg_BM (_.contribob));}
   _.contnamev = objcontributornamepayl_BM (_.contribob); bool popenfile = mailhtml_file_bm[0] == '|'; FILE * infil = popenfile //
   ...
   FATAL_BM ("asprintf failure for contributor %s - %m",
             mailhtml_contributor_bm); size_t linsiz = 0;
   do
   {
   int linlen = getline (&linbuf, &linsiz, infil); if (linlen < 0) break;
      /**** 
   ...
   char *pi = NULL;
   char *npc = NULL;
   const char *repl = NULL; while ((pi = strstr (pc, "<?bismon")) != NULL)
   {
   char smallbuf[64];
   int lnpi = 0; npc = NULL; memset (smallbuf, 0, sizeof (smallbuf));
#define MAIL_WITH_PI_bm(CurPi)			\
	  ((lnpi=strlen(CurPi))>0		\
	   && !strncmp(pi,CurPi,lnpi)		\
	   && (npc=pi+lnpi))
   if (MAIL_WITH_PI_bm ("<?bismon-contributor?>")) repl = dyncontrib;
   else
   if (MAIL_WITH_PI_bm ("<?bismon-subject?>")) repl = mailhtml_subject_bm;
   else
   if (MAIL_WITH_PI_bm ("<?bismon-attachment?>"))
   repl = mailhtml_attachment_bm;
   else
   if (MAIL_WITH_PI_bm ("<?bismon-pid?>"))
   {
   snprintf (smallbuf, sizeof (smallbuf),
             "pid %ld", (long) getpid ()); repl = smallbuf;}
   else
   if (MAIL_WITH_PI_bm ("<?bismon-host?>")) repl = myhostname_BM;
   else
   if (MAIL_WITH_PI_bm ("<?bismon-lastgitcommit?>"))
   repl = bismon_lastgitcommit;
   else
   if (MAIL_WITH_PI_bm ("<?bismon-timestamp?>")) repl = bismon_timestamp;
   else
   {
   npc = pc + strlen ("<?bismon"); repl = "<?bismon";}
   fwrite (prevpc, pi - prevpc, 1, bufil);
   fputs (repl, bufil); prevpc = pi + lnpi; pc = npc;};
   fwrite (pc, linbuf + linlen - pc, 1, bufil);
#undef MAIL_WITH_PI_bm
   pc = npc;}
   while (!feof (infil)); fflush (bufil); long buflen = ftell (bufil); DBGPRINTF_BM ("do_test_mailhtml_bm buflen=%ld; bufzon:\n%s\n", buflen, bufzon); if (!bufzon || buflen < 0 || buflen > (int) bufsiz) FATAL_BM ("do_test_mailhtml_bm corrupted bufil (bufzon@%p buflen=%ld bufsiz=%ld) %m", bufzon, buflen, (long) bufsiz); bufzon[buflen] = (char) 0; send_html_email_to_contributor_BM (mailhtml_subject_bm, bufzon, mailhtml_attachment_bm, _.contribob, CURFRAME_BM); fclose (bufil); free (bufzon), bufzon = NULL; free (linbuf), linbuf = NULL; free (dyncontrib), dyncontrib = NULL; linsiz = 0;}   /* end do_test_mailhtml_bm */
   ...
   {
#endif // __cplusplus
   const char *onion_version (void); const char *jansson_version_str (void);
#ifdef __cplusplus
   };
#endif //__cplusplus
////////////////////////////////////////////////////////////////
   void give_prog_version_BM (const char *progname)
   {
   printf ("%s: version information\n", progname); printf ("\t timestamp: %s\n", bismon_timestamp); printf ("\t git id: %s\n", bismon_gitid); printf ("\t last git commit: %s\n", bismon_lastgitcommit); printf ("\t last git tag: %s\n", bismon_lastgittag); printf ("\t source checksum: %s\n", bismon_checksum); printf ("\t source dir: %s\n", bismon_directory); printf ("\t GNUmakefile: %s\n", bismon_gnumakefile); printf ("\t Glib (see developer.gnome.org/glib) version: %u.%u.%u\n", glib_major_version, glib_minor_version, glib_micro_version); printf ("\t Libonion - see github.com/davidmoreno/onion - version: %s\n", onion_version ()); printf ("\t libjansson - see digip.org/jansson/ - version: %s\n", jansson_version_str ()); printf ("########\n"); printf ("run\n" "\t   %s --help\n" "to get help.\n", progname); exit (EXIT_SUCCESS);}      /* end give_prog_version_BM */

   void
   backtracerrorcb_BM (void *data __attribute__((unused)),
                       const char *msg, int errnum)
   {
   backtracestate_BM = NULL; FATAL_BM ("backtrace error: %s #%d", msg, errnum);}        /* end backtracerrorcb_BM */
...
/// see https://github.com/ianlancetaylor/libbacktrace
   ...
   {
   struct backtrace_state *state; FILE * f;};
/* Print one level of a backtrace.  */
   ...
   char nambuf[80]; memset (nambuf, 0, sizeof (nambuf)); if (funame)
   {
   ...
   if ((commentv = objgetattr_BM (fnob, BMP_comment)) != NULL
       && (isstring_BM (commentv)))
   {
   const char *combytes = bytstring_BM (commentv); int comlen = lenstring_BM (commentv); const char *eol = strchr (combytes, '\n'); if (eol && eol > combytes) comlen = eol - combytes - 1; fprintf (pdata->f, "0x%lx %s\n\t"   //
                                                                                                                                                                                                     "€%.9s |%*s|\n",
                                                                                                                                                                                                     (unsigned
                                                                                                                                                                                                      long)
                                                                                                                                                                                                     pc,
   ...
   {
   Dl_info di; memset (&di, 0, sizeof (di)); if (dladdr ((void *) pc, &di))
   {
   if (di.dli_sname)
   {
   ...
   fprintf (pdata->f, "0x%lx ???\n", (unsigned long) pc);}
   if (filename) fprintf (pdata->f, "\t%s:%d\n", basename (filename), lineno); return 0;}       /* end printbt_callback_BM */
...
/* Print a backtrace.  */
   ...
   void backtrace_print_BM (struct backtrace_state *state, int skip, FILE * f)
   {
   struct print_data_BM data; data.state = state; data.f = f; backtrace_full (state, skip + 1, printbt_callback_BM, errorbt_callback_BM, (void *) &data);}      /* end backtrace_print_BM */

////////////////////////////////////////////////////////////////
   void log_begin_message_BM (void)
   {
   extern void onion_log_begin_message_BM (void); if (web_is_running_BM)
   {
   onion_log_begin_message_BM (); return;}
   /// FIXME: this is probably wrong, log_begin_message_BM can run with
   /// event loop...
#warning log_begin_message_BM could be wrong
   FATAL_BM ("log_begin_message_BM without web");}      /* end log_begin_message_BM */



   void log_end_message_BM (void)
   {
   extern void onion_log_end_message_BM (void); if (web_is_running_BM)
   {
   onion_log_end_message_BM (); return;}
   /// FIXME: this is probably wrong, log_end_message_BM can run with
   /// event loop...
#warning log_begin_message_BM could be wrong
   FATAL_BM ("log_end_message_BM without web");}        /* end log_end_message_BM */



   void log_puts_message_BM (const char *str)
   {
   extern void onion_log_puts_message_BM (const char *);
   if (web_is_running_BM)
   {
   onion_log_puts_message_BM (str); return;}
   /// FIXME: this is probably wrong, log_end_message_BM can run with
   /// event loop...
#warning log_puts_message_BM could be wrong
   FATAL_BM ("log_puts_message_BM without web or GUI for: %s", str);}   /* end log_puts_message_BM */

   void log_object_message_BM (const objectval_tyBM * obj)
   {
   extern void onion_log_object_message_BM (const objectval_tyBM *);
   if (web_is_running_BM)
   {
   onion_log_object_message_BM (obj); return;}
   /// FIXME: this is probably wrong, log_object_message_BM can run with
   /// event loop...
#warning log_object_message_BM could be wrong
   FATAL_BM ("log_object_message_BM without web or GUI for %s", objectdbg_BM (obj));}   /* end log_object_message_BM */

   void log_printf_message_BM (const char *fmt, ...)
   {
   char smallbuf[64];
   ...
   va_end (args); if (ln >= (int) sizeof (smallbuf) - 1)
   {
   buf = calloc ((prime_above_BM (ln + 2) | 7) + 1, 1);
   if (!buf)
   FATAL_BM ("failed to calloc for %d bytes (%m)", ln);
   va_start (args, fmt); vsnprintf (buf, ln + 1, fmt, args); va_end (args);}
   log_puts_message_BM (buf); if (buf != smallbuf) free (buf);} /* end log_printf_message_BM */



   void parse_program_options_BM (int argc, char **argv)
   {
   ...
   g_option_context_free (gctx); if (onion_web_base_BM && *onion_web_base_BM)
   {
   INFOPRINTF_BM
   ("%s: will run web service with onion web base %s (pid #%d, parent pid #%d, host %s)",
    argv[0], onion_web_base_BM, (int) getpid (), (int) getppid (),
    myhostname_BM); run_onion_BM = true;}; if (unix_json_socket_BM)
   {
   INFOPRINTF_BM
   ("%s: should serve UNIX socket %s for JSONRPC (pid #%d, host %s, git %s)",
    ...
    bismon_shortgitid);};
   ///
   DBGPRINTF_BM ("end parse_program_options_BM, argc wad %d now %d", initialargc, argc);}       /* end parse_program_options_BM */


   const char *get_real_executable_path_BM (void)
   {
   return real_executable_BM;}  /* end get_real_executable_path_BM */


   void write_pid_into_file_and_kill_old_BM (const char *pidfilepath)
   {
   ASSERT_BM (pidfilepath && pidfilepath[0]);
   int oldpid = -1; FILE * oldpidfile = fopen (pidfilepath, "r");
   /// if the pidfile starts with a dot or a percent, dont kill the old process
   bool shouldkillold = pidfilepath[0] != '.' && pidfilepath[0] != '%';
   bool killedold = false; if (oldpidfile)
   {
   ...
   if (!readlink
       ...
       && strlen (thisexepath) < sizeof (thisexepath) - 2 && shouldkillold)
   {
   INFOPRINTF_BM
   ("quitting old Bismon process of pid %d running this executable %s",
    oldpid, thisexepath);
   killedold = 0 == kill (oldpid, SIGQUIT); if (killedold)
   {
   INFOPRINTF_BM ("SIGQUIT-ed old Bismon process of pid %d on %s running this executable %s", oldpid, myhostname_BM, thisexepath); fflush (NULL); sleep (1);    /* to let the other Bismon die gracefully */
   }
   else
   WARNPRINTF_BM
   ...
   fprintf (pidfile, "#built %s\n", bismon_timestamp); fflush (pidfile);
   /// On purpose the pid file is kept open.... this ensures that lsof(1)
   /// might be used...
   INFOPRINTF_BM ("wrote pid %d in pid-file %s", (int) getpid (), pid_filepath_bm);}    /* end write_pid_into_file_and_kill_old_BM */

   void test_make_empty_sigusr1_dump_dir_BM (void)
   {
   ...
   FATAL_BM ("failed to mkdir %s - %m", dirbufname); INFOPRINTF_BM ("Successfully made empty SIGUSR1 dump directory %s", dirbufname);}  /* end test_make_empty_sigusr1_dump_dir_BM */
